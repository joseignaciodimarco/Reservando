...
Listado.prototype.buscarRestaurante = function(id) {   
    return  this.restaurantes.find(restaurante => restaurante.id === id) != undefined ? 
            this.restaurantes.find(restaurante => restaurante.id === id): "No se ha encontrado ningún restaurant";
}
...
Listado.prototype.obtenerCiudades = function() {
    ...
    return eliminaRepetidos(ciudades);
}
...
Listado.prototype.obtenerRubros = function() {
    ...
    return eliminaRepetidos(rubros);
}
...
Listado.prototype.obtenerHorarios = function() {
    ...
    var horariosRestaurant = [];

    arregloHorarios.map(horario => horario.map(hora => horariosRestaurant.push(hora)));

    return eliminaRepetidos(horariosRestaurant);
